 ...
 */
public class AggregatedSGSuccessAndFailedMetric<Peer> implements Metric<Peer, Object>{
	...
	/** This is our current measures map where we save the {@link OutcomeAgg} per peer. */
	private Map<Peer, OutcomeAgg> peerMap;
	 ...
	 * @param unit
	 */
	public AggregatedSGSuccessAndFailedMetric(String name, String unit) {
		super();
		this.name = name;
		this.unit = unit;
	...
	public List<String> getHeadlines() {
		 ...
		 */
		return Arrays.asList("AggregatedSuccessful", "AggregatedFailed");
	}
	...
	public List<String> getPeerMeasurementValues(Object peer) {
		...
		OutcomeAgg peerOutcome = this.peerMap.getOrDefault(peer, new OutcomeAgg());
	...
	public void addSearchFinished(Peer contact,
		...
		// fill the peer map
		OutcomeAgg peerOutcome = this.peerMap.getOrDefault(contact, new OutcomeAgg());
	...
	public void resetTimeMeasurement() {
		/*
		 * here, you can reset your time measurement after each analyzer step.
		 * For this example we do not reset any data as we want aggregated data.
		 * -> NOP
		 */
	}
	...
	public void resetPeerMeasurement() {
		/*
		 * just like resetTimeMeasurement()
		 * -> NOP
		 */
	}
	...
	private void generatePlot(Writer script, String analyzer,
		...
		script.write("plot '" + analyzer
				...
				+ " ls 2 title '" + "Aggregated Success" + "' with linespoints,"
				+ "'" + analyzer
				+ AbstractFileAnalyzer.FILE_EXTENSION + "' using 1:"
				+ (startIndex+1)	// the second column is aggregated failed!
				+ " ls 1 title '" + "Aggregated Failed" + "' with linespoints"
	 ...
	 * 
	 * @author Andreas Disterhöft
	 * @version 1.0, 01/27/2017
	 */
	private static class OutcomeAgg {
		...
		OutcomeAgg(int successCounter, int failedCounter) {
			super();
